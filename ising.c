...
// Change in energy for one flipped spin
double delta_E(unsigned int i, unsigned int j, int grid[num_cells_x][num_cells_y]) {
    double result = 2.0*H*mu_b*(double)grid[i][j];
    result += 2.0*J*grid[i][j]*(grid[mod_neg(i-1, num_cells_x)][j] + grid[(i+1)%num_cells_x][j] + 
                              grid[i][mod_neg(j-1, num_cells_y)] + grid[i][(j+1)%num_cells_y]);
    return result;
}

// Chance that spin will flip
